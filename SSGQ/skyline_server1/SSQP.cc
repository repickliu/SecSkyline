...
int secComp( int u1, int v1) {
    ...
    for(int i = 0; i < l ;i++){
        ...
        d[i]=secMul((p[i]%2+2)%2,(q[i]%2+2)%2, 4500);
    }

    c[0]=d[0];
    for(int i =1; i < l-1;i++){
        d[i]=(((d[i]+PartyNum)%2)+2)%2;
        e[i]=((secMul(w[i],c[i-1], 4500)+PartyNum)%2+2)%2;
        c[i]=((secMul(e[i],d[i], 4500)+PartyNum)%2+2)%2;
    }
...
vector<vector<int>> secMap(vector<vector<int>>& P, vector<int>& q){
    ...
    for(int i = 0; i < n; ++i){
        for(int j = 0; j < m; ++j){
            ...
            // int b_ = PartyNum - b;
            T[i][j] = secMul(P[i][j] - q[j], P[i][j] - q[j], 4500);
        }
    }
    return T;
}
...
sk_r SecFetch(vector<int>& s, vector<vector<int>>& T, vector<vector<int>>& P){
    ...
    for(int i = 1; i < s.size(); ++i){
        ...
        int phai = secComp(s[i], sMin, 4500);
        int phai_ = PartyNum - phai;
        sMin = secMul(phai, s[i], 4500) + secMul(phai_, sMin, 4500);
        // cout << "sMin" << endl;
        for(int j = 0; j < t_.size(); ++j){
            t_[j] = secMul(phai, T[i][j], 4500) + secMul(phai_, t_[j], 4500);
            // cout << "t_[j]" << endl;
        }
        for(int j = 0; j < p_.size(); ++j){
            p_[j] = secMul(phai, P[i][j], 4500) + secMul(phai_, p_[j], 4500);
            // cout << "p_[j]" << endl;
        }
    }
    sk_r sk(t_, p_, sMin);
...
vector<int> SecFilt(vector<vector<int>>& T, vector<int>& t, int sMin, vector<int>& sum){
    ...
    for(int i = 0; i < sum.size(); ++i){
        ...
        int theta = PartyNum - secComp(sMin, sum[i], 4500);
        int isFirstSky = secMul(PartyNum - flag, theta, 4500);
        flag = isFirstSky + flag - 2 * secMul(isFirstSky, flag, 4500);
        int th = 0;
        vector<int> th_s(m);
        for(int j = 0; j < m; ++j){
            // th_s[j] = PartyNum - secComp(t[j], T[i][j], PartyNum, ref(sock));
            th_s[j] = PartyNum - secComp(T[i][j], t[j], 4500);
        }
        for(int j = 0; j < m; ++j)
            th = secMul(th, th_s[j], 4500);
        // cout << "th = " << th << endl;
        int isDomi = secMul(th, PartyNum - theta, 4500);
        int fi = isFirstSky + isDomi - 2 * secMul(isFirstSky, isDomi, 4500);
        int vMax;
        if(PartyNum == 1)
            vMax = vMax_a;
        else
            vMax = vMax_b;
        s[i] = secMul(fi, vMax, 4500);
        s[i] += secMul(PartyNum - fi, sum[i], 4500);
...
vector<vector<int>> SSQP(vector<vector<int>>& P, vector<int>& q){
    ...
    while(!isStop){
        ...
        isStop = PartyNum - secComp(sk.sMin, vMax, 4500);
...
void test_SSQP(){
    ...
    cout << endl;
    cout << "--------------------------------" << endl;
    vector<vector<int>> skr = SSQP(P_a, q_a);
    cout << endl;
    sendresult(skr, P2, 7000);
    cout << endl;
}
