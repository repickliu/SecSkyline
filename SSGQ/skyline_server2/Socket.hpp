...
namespace Socket
{
    ...
    public:
        ...
        void send_matric(Ip ip, Port port, vector<vector<vector<int>>>& K);
    ...
    void UDP::send_matric(Ip ip, Port port, vector<vector<vector<int>>>& K){
        struct sockaddr_in address;
        address.sin_family = AF_INET;
        address.sin_port = htons(port);
        inet_aton(ip.c_str(), &address.sin_addr);
        ...
        col = line > 0 ? K[0].size() : 0;
        m = K[0][0].size();
        cout << line * col * m << endl;
        vector<int> V(line * col * m);
        int t = 0;
        for(int i = 0; i < line; ++i)
            for(int j = 0; j < col; ++j)
                for(int k = 0; k < m; ++k)
        ...
        if(!line){
            ...
            dataSizeToSend = to_string(line) + " " + to_string(col)+" "+to_string(m);
        if (sendto(this->_socket_id, (void*)dataSizeToSend.c_str(), sizeof(dataSizeToSend) + 1, 0, (struct sockaddr*)&address, sizeof(struct sockaddr_in)) == -1)
        {
            stringstream error;
            error << "[send] with [ip=" << ip << "] [port=" << port << "] [data=" << dataSizeToSend << "] Cannot send";
    ...
    vector<vector<int>> UDP::receive_matric()
        {
            ...
            if(l_c == "null"){
                ...
                recv_all(receivedData.data(), line * col * sizeof(int), 0, (struct sockaddr*)&address, (socklen_t*)&size);
                // cout << receivedData.size() << endl;
                for (int i = 0; i < line; ++i) {
                    for (int j = 0; j < col; ++j) {
